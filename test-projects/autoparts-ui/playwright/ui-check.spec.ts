...
test.describe('Visual Check', () => {
  ...
  for (const bp of BREAKPOINTS) {
    test(`Screenshot at ${bp.name} (${bp.width}px)`, async ({ page }) => {
      ...
      // Look for actual content instead of skeleton placeholders
      await page.waitForTimeout(1500); // Wait for simulated loading

      // Or wait for a specific element that indicates content is loaded
      try {
        await page.waitForSelector('h1, h2, [data-loaded="true"]', { timeout: 5000 });
      } catch {
        console.log('Note: No heading found - page may still be loading');
      }
